...
async function onSubmit() {
  ...
  loading.value = true;
  try {
    const result = await iamStore.loginHealthEntity(email.value, password.value);
    if (!result.ok) {
  ...
  <auth-panel show-back @back="router.push({ name: 'login' })">
    <div class="reg-compact">
      ...
      <p class="reg-compact__footer">
        ...
        <button type="button" class="link-btn" @click="router.push({ name: 'iam-register-health-entity' })">
          {{ t('iam.login.signUp') }}
        </button>
      </p>
    </div>
